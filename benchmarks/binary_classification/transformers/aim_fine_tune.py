...
from transformers import AutoImageProcessor, AIMForImageClassification
...
def load_model(model_path, labels, pretrained=True):
    ...
    if pretrained:
        ...
        model.head.linear = torch.nn.Linear(1536, 2)
        model = model.cuda()
    else:
        ...
        model = AIMForImageClassification.from_config(config)
...
if __name__ == '__main__':
    ...
    train_dataset = datasets.ImageFolder(os.path.join(args.data_dir,"Train/"), transform=val_transforms)
    valid_dataset = datasets.ImageFolder(os.path.join(args.data_dir,"Valid/"), transform=val_transforms)
    test_dataset = datasets.ImageFolder(os.path.join(args.data_dir,"Test/"), transform=val_transforms)
    ...
    for epoch in range(starting_epoch, args.num_epochs):
        ...
        model.train()
        for step, batch_labels in enumerate(train_dataloader):
